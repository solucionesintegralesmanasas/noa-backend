...
/**
 * Solicitud de validación para la actualización de un Proyecto.
 */
class UpdateProjectRequest extends FormRequest
{
    ...
    {
        return [
            'company_uuid' => ['sometimes', 'uuid', 'exists:companies,uuid'],
            'project_name' => [
                'sometimes',
                'string',
                'max:255',
                Rule::unique('projects')->ignore($this->route('uuid'), 'uuid')
                    ->where(function ($query) {
                        return $query->where('company_uuid', $this->company_uuid ?? $this->input('company_uuid'));
                    }),
            ],
            'start_date' => ['sometimes', 'date'],
            'completion_date' => ['sometimes', 'date', 'after_or_equal:start_date'],
            'project_value' => ['nullable', 'numeric', 'min:0'],
            'purchase_order' => ['nullable', 'string', 'max:255'],
            'purchase_order_file' => ['nullable', 'file', 'mimes:pdf', 'max:10240'],
            'remove_purchase_order' => ['sometimes', 'boolean'],
     ...
     * Se evalúa sobre el estado final del proyecto (lo enviado + lo ya asignado).
     */
